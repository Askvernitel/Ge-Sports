...
/** Plain sign-up: email, password, display name. No PUBG name and no wallet
 * here — those are linked later from Profile, once the player actually has
 * an account. */
export function RegisterPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setBusy(true);
    try {
      await register(email, password, displayName.trim() || email.split('@')[0] || 'Player');
    ...
    <div className="min-h-screen grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(340px,1fr))' }}>
      <div className="p-10 py-12 flex flex-col justify-between">
        ...
        <div className="max-w-[420px] py-12">
          <h1 className="font-display font-black text-6xl tracking-[1px] uppercase leading-[0.95] mb-4">
            Create account
          </h1>
          <p className="text-md leading-[1.6] mb-9">
            Register with an email and password. Wallet connection and PUBG name linking happen later, from your
            profile, before you join a room.
          </p>
