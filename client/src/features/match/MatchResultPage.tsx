...
export function MatchResultPage() {
  ...
  return (
    <div>
      ...
      <div className="m-6">
        <div
          className="grid pb-2.5 border-b border-lichen px-4"
        ...
        {result.rows.map((row) => (
          <div
            key={row.rank}
            className="grid items-center px-4 py-3.5 bg-panel mt-0.5"
            style={{ gridTemplateColumns: '60px 1fr 90px 90px 110px' }}
          >
            <span className="font-display font-bold text-lg">{row.rank}</span>
            <span className="text-base">{row.player}</span>
            <span className="font-mono text-base tabular-nums">{row.kills}</span>
            <span className="font-mono text-base tabular-nums text-lichen">{row.points}</span>
            <span className="font-mono text-md tabular-nums">{formatTokens(row.payout)}</span>
          </div>
        ))}

        <div className="flex justify-end gap-8 px-4 py-5 mt-2 border-t border-lichen font-mono text-base">
